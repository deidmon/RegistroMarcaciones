...
module.exports = function (dbInjected) {
  ...
  async function consultUser(user, password) {
    ...
    return bcrypt.compare(password, data.Contraseña).then((resultado) => {
      if (resultado == true) {
  ...
  async function consultMarkMonth(idUser, date) {
    ...
    const dataMonth = await db.queryMarkMonth(
      tabletypeValidation,
      tableAssist,
      idUser,
      date,
  ...
  async function consultMarkWeek(idUser) {
    ...
    const dataWeek = await db.queryMarkWeek(
      tableAssist,
      tabletypeValidation,
      idUser
    );
    if (!dataWeek) {
      message =
        "Actualmente, no se han registrado asistencias para esta semana.";
      return { messages: message };
    } else {
      return dataWeek;
    }
  ...
  async function consultMarkDay(idUser, date) {
    ...
    const dataDay = await db.queryMarkDay(
      tableAssist,
      tableTypeMarking,
      tabletypeValidation,
      tablePermissions,
  ...
  async function allWorkers(body) {
    ...
    if (IdRolUser === 3 || IdRolUser === 4) {
      return db.queryAllWorkers(
        ...
        body.CIP,
        body.DNI,
        body.IdEstateWorkerA ?? 1,
        body.IdEstateWorkerI ?? 2,
        PageSiize,
        getOffset
      );
    }
    ...
    return db.queryAllWorkersByUser(
      tableUser,
      tableStateUser,
      tableModalityWork,
      tableRol,
  ...
  async function getWorkersCounter(body) {
    ...
    let result;
    if (IdRolUser === 3 || IdRolUser === 4) {
      result = await db.queryGetWorkersCounter(
        tableUser,
        body.name ?? "",
        body.CIP,
        body.DNI,
      ...
      result = await db.queryGetWorkersCounterByUser(
        tableUser,
        body.name ?? "",
        body.CIP,
        body.DNI,
  ...
  async function getLeadersCounter(body) {
    result = await db.queryGetLeadersCounter(
      tableUser,
      body.name,
      body.CIP,
      body.DNI,
  ...
  async function activateUsers(body) {
    ...
    const respuesta = await db.queryActivateUsers(
      tableUser,
      body.status,
      body.idUsers
    );
  ...
  async function updatePasswordOfUser(body) {
    ...
    if (!hasNumber || !hasCharacter || !hasUppercase) {
      return {
        messages:
          "La contraseña debe contener al menos un número, un carácter y una mayúscula",
      };
    }

    if (body.password.length < constant.minimumPasswordCharacters) {
      return {
        messages: `La  contraseña debe tener como minimo ${constant.minimumPasswordCharacters} caracteres`,
      };
    }
  ...
  /* 📌 Actualizar contraseña cuando lo olvido*/
  async function updatePasswordOfUserForgot(body) {
    // Expresiones regulares para validar si la contraseña contiene al menos un número, un carácter y una mayúscula
    const hasNumber = /\d/.test(body.password);
    const hasCharacter = /[a-zA-Z]/.test(body.password);
    const hasUppercase = /[A-Z]/.test(body.password);
    ...
    let response = await db.queryVerificationOfCode(body.code, body.idUser);
    ...
    if (response && response.length > 0) {
      if (
        response[0].state == constant.stateActive &&
        fechaActual <= response[0].time_exp
      ) {
        ...
        await db.queryUpdateAnyTable(
          constant.tableCodeUser,
          toUpdate,
          whereToUpdate
        );
    ...
    //Termina verificación de código
    let password = body.password;
  ...
  async function sendCodeVerfication(body) {
    ...
    const verifiExistRegister = await db.queryGetWhere(
      constant.tableCodeUser,
      whereVerified
    );
    ...
    const responseEmail = await helpers.sendCodeVerificationOutlook(
      response[0].Email,
      code_user
    ); //Envia el correo con el código
  ...
  async function consultUserArea(id) {
    ...
    const dataUserArea = await db.queryConsultUserArea(
      constant.tableRelationshipAreas,
      constant.tableCompany,
      constant.tableManagement,
      constant.tableHeadquarters,
      constant.tableUnits,
      codigoUserArea
    );
    if (!dataUserArea) {
